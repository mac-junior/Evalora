...
const StudentSidebar = ({ onClose }) => {
  ...
  const navItems = [
    ...
    { to: '/student/assessments', icon: BookOpen, label: 'Assessments' },
  ...
  return (
    <div className={`h-full flex flex-col transition-all duration-300 border-r ${
      ...
      <div className="flex items-center justify-between px-5 py-6 border-b" style={{ borderColor: '#E0D3C5' }}>
        <Logo to="/student/dashboard" size="default" showText={!collapsed} />
      ...
      {!collapsed && studentUser && (
        <div className="px-3 py-4 border-t" style={{ borderColor: '#E0D3C5' }}>
          <div className="flex items-center gap-3 px-3 py-2">
            ...
            >
              {studentUser.profile_pic ? (
                <img src={studentUser.profile_pic} alt="" className="w-full h-full object-cover" />
              ) : (
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate" style={{ color: '#5C4B3A' }}>@{studentUser.username}</p>
              <p className="text-xs truncate" style={{ color: '#7A6B5D' }}>{studentUser.matricule}</p>
            </div>
          </div>
        </div>
      )}
